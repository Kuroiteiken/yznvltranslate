...
class MLTerminologyExtractor:
    ...
    def get_untranslated_files_text(self, target_token_count: int | None = None, margin=0.05):
        ...
        upper_limit = target_token_count * (1 + margin) # 472500
        ...
        for file in dwnld_files:
            ...
            if acc_tokens + current_tokens > upper_limit:
                logger.info(f"'{file}' ile token limiti ({upper_limit}) %5 sapma dahil aşılıyor. Bu bölüm çıkarıldı ve ekleme durduruluyor.")
                break
            ...
            if acc_tokens >= target_token_count:
                logger.info(f"Hedef token sayısına ({target_token_count}) ulaşıldı veya yaklaşıldı. (Güncel: {acc_tokens})")
                break
                
        logger.info(f"Toplanan metin için tahmini token sayısı: {acc_tokens}")
        return acc_text

    def run(self, append: bool = False):
        ...
        combined_text = self.get_untranslated_files_text(margin=0.05)
